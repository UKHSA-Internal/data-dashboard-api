...
from validation import age, is_public


class TestAPIHeadlineManager:
    @mock.patch.object(APIHeadlineManager, "query_for_superseded_data")
    def test_delete_superseded_data(
        ...
        When `delete_superseded_data` is called
            from an instance of the `APIHeadlineManager`
        ...
        # When
        APIHeadlineManager().delete_superseded_data(
            theme=fake_theme,
        ...
        # Then
        spy_query_for_superseded_data.assert_called_once_with(
            theme=fake_theme,
            sub_theme=fake_sub_theme,
            topic=fake_topic,
            metric=fake_metric,
