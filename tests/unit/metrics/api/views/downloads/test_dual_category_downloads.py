...
class TestDualCategoryDownloadsView:
    ...
    def test_get_serializer_class_returns_headline_serializer_without_confidence_intervals(
        self,
    ):
        """
        Given a headline metric group queryset
        When `_get_serializer_class()` is called on `DualCategoryDownloadsView`
        Then a headline serializer is returned with confidence intervals disabled
        """
        # Given
        view = DualCategoryDownloadsView()
        ...
        # When
        serializer = view._get_serializer_class(
            queryset=queryset,
            metric_group="headline",
        )

        # Then
        assert serializer.context["confidence_intervals"] is False

    @mock.patch(
        "metrics.api.views.downloads.single_category_downloads."
        "SingleCategoryDownloadsView._get_serializer_class"
    )
    def test_get_serializer_class_delegates_to_parent_for_timeseries(
        self,
        mocked_parent_get_serializer: mock.MagicMock,
    ):
        """
        Given a timeseries metric group queryset
        When `_get_serializer_class()` is called on `DualCategoryDownloadsView`
        Then the parent downloads view serializer is returned
        """
        # Given
        view = DualCategoryDownloadsView()
        queryset = mock.MagicMock()
        parent_serializer = mock.MagicMock()
        mocked_parent_get_serializer.return_value = parent_serializer
        ...
        # Then
        mocked_parent_get_serializer.assert_called_once_with(
            queryset=queryset,
            metric_group="cases",
        )
        assert serializer is parent_serializer
