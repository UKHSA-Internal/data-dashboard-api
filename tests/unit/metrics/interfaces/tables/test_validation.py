...
class TestValidate:
    ...
    @mock.patch.object(
        validation.TablesRequestValidator,
        "_validate_metric_is_available_for_topic",
    )
...
class TestValidateMetricIsAvailableForTopic:
    def test_can_validate_successfully(self):
        """
        Given a metric_name of `covid_occupied_beds_latest` and a topic_name of `COVID-19` which is valid
        ...
        fake_metric_manager = FakeMetricManager(metrics=metric_names)
    ...
    def test_can_raise_error_for_invalid_combination(self):
        """
        Given a metric_name of `covid_occupied_beds_latest` and a topic_name of `Influenza` which is invalid
        When `_validate_metric_is_available_for_topic()` is called from an instance of `TablesRequestValidator`
...
class TestDoesMetricHaveMultipleRecords:
    def test_returns_true_for_multiple_records(self):
        ...
        # When
        metric_name_has_multiple_records: bool = (
            validator._does_metric_have_multiple_records()
        )

        # Then
        assert metric_name_has_multiple_records

    def test_returns_false_for_single_record(self):
        """
        Given a metric_name of `weekly_positivity` and a topic_name of `Influenza`
        ...
        # Then
        assert not metric_name_has_multiple_records

    def test_returns_false_for_non_existent_record(self):
        """
        Given a metric_name and a topic_name for a record which does not exist
        When `does_metric_have_multiple_records()` is called from an instance of `TablesRequestValidator`
        Then False is returned
        """
        # Given
