...
class TestCMSDraftPagesViewSet:
    @patch("cms.dashboard.viewsets.loads", return_value={"exp": 9999999999, "page_id": 2})
    def test_detail_view_returns_401_if_page_id_mismatch(self, mock_loads):
        """Covers the branch where payload_page_id does not match instance.pk (line 112)."""
        ...
        viewset.get_queryset = MagicMock(return_value=MagicMock(filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=fake_instance)))))
        response = viewset.detail_view(request=request, pk=1)
        assert response.status_code == 401
    @patch("cms.dashboard.viewsets.loads", return_value={"exp": 9999999999, "page_id": 1})
    def test_detail_view_returns_200_with_latest_revision(self, mock_loads):
        """Returns 200 and latest revision if present."""
        viewset = CMSDraftPagesViewSet()
        request = MagicMock(spec=Request)
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        # Mock queryset and instance
        fake_instance = MagicMock()
        fake_instance.get_latest_revision.return_value = MagicMock(as_object=MagicMock(return_value="draft_page"))
        viewset.get_queryset = MagicMock(return_value=MagicMock(filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=fake_instance)))))
    ...
    def test_detail_view_returns_200_with_published_version(self, mock_loads):
        """Returns 200 and published version if no latest revision."""
        viewset = CMSDraftPagesViewSet()
        request = MagicMock(spec=Request)
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        fake_instance = MagicMock()
        fake_instance.get_latest_revision.return_value = None
        viewset.get_queryset = MagicMock(return_value=MagicMock(filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=fake_instance)))))
        viewset.get_serializer = MagicMock(return_value=MagicMock(data={"foo": "published"}))
        response = viewset.detail_view(request=request, pk=1)
        assert response.status_code == 200
    ...
    def test_detail_view_returns_404_if_page_not_found(self, mock_loads):
        ...
        request = MagicMock(spec=Request)
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        viewset.get_queryset = MagicMock(return_value=MagicMock(filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=None)))))
        response = viewset.detail_view(request=request, pk=1)
    ...
    @patch("cms.dashboard.viewsets.loads", side_effect=BadSignature)
    def test_detail_view_returns_401_if_bad_signature(self, mock_loads):
        viewset = CMSDraftPagesViewSet()
        request = MagicMock(spec=Request)
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        response = viewset.detail_view(request=request, pk=1)
    ...
    def test_detail_view_returns_401_for_expired_token(self, mock_loads):
        """Returns 401 if the token is expired (exp in past)."""
        request = MagicMock(spec=Request)
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        viewset = CMSDraftPagesViewSet()
        response = viewset.detail_view(request=request, pk=1)
    ...
    def test_detail_view_previews_disabled_returns_403(self):
        """Covers the 403 path when PAGE_PREVIEWS_ENABLED is False."""
        viewset = CMSDraftPagesViewSet()
        request = MagicMock(spec=Request)
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        with patch("cms.dashboard.viewsets.settings") as mock_settings:
            mock_settings.PAGE_PREVIEWS_ENABLED = False
            response = viewset.detail_view(request=request, pk=1)
        assert response.status_code == 403
        assert "Page previews are disabled" in response.data["detail"]

    def test_get_object_calls_super_when_forced_page_none(self):
        """Covers get_object() when _forced_detail_page is None, falling back to super().get_object()."""
        viewset = CMSDraftPagesViewSet()
        viewset._forced_detail_page = None

        # Patch the parent method
        with patch("cms.dashboard.viewsets.PagesAPIViewSet.get_object", return_value="parent_obj") as mock_parent:
    ...
    def test_happy_path_returns_200(self, mock_loads):
        ...
        request.headers = {"x-draft-auth": "Bearer faketoken"}
        fake_instance = MagicMock()
        fake_instance.get_latest_revision.return_value = MagicMock(as_object=MagicMock(return_value=fake_instance))
