...
class TestAddFrontendPreviewActionExceptions:
    ...
    def test_no_reverse_match_fallback_template(self, mock_reverse, mock_view):
        ...
        """
        page = SimpleNamespace(pk=1, slug="slug", custom_preview_enabled=True, has_unpublished_changes=True, live=False)
        menu_items = []
        # Patch build_route_slug to return a known slug
        mock_view.build_route_slug.return_value = "slug"
        with mock.patch("cms.dashboard.wagtail_hooks.settings", PAGE_PREVIEWS_FRONTEND_URL_TEMPLATE="http://test/{slug}", PAGE_PREVIEWS_ENABLED=True):
...
class FakePage:
    ...
    def __init__(self, pk=1, slug="foo", can_edit=True, has_unpublished_changes=True, live=False):
        self.pk = pk
        self.slug = slug
        self._can_edit = can_edit
...
class TestFrontendPreviewButton:
    def test_non_edit_view_returns_empty(self):
        page = FakePage()
        buttons = wagtail_hooks.frontend_preview_button(page=page, user=None, next_url=None, view_name="index")
        assert buttons == []
    ...
    def test_fallback_uses_template(self, spy_reverse: mock.MagicMock, settings):
        ...
        settings.PAGE_PREVIEWS_FRONTEND_URL_TEMPLATE = "https://frontend.test/preview?slug={slug}"
        page = SimpleNamespace(pk=123, slug="bar", custom_preview_enabled=True, has_unpublished_changes=True, live=False)
        buttons = wagtail_hooks.frontend_preview_button(page=page, user=None, next_url=None, view_name="edit")
    ...
    def test_live_non_draft_page_label_is_view_live(self):
        page = SimpleNamespace(pk=123, slug="bar", custom_preview_enabled=True, has_unpublished_changes=False, live=True)
        buttons = wagtail_hooks.frontend_preview_button(page=page, user=None, next_url=None, view_name="edit")
        assert isinstance(buttons, list) and buttons

        assert buttons[0].label == "View Live"
    def test_page_with_no_draft_and_no_live_has_no_button(self):
        page = SimpleNamespace(pk=123, slug="bar", custom_preview_enabled=True, has_unpublished_changes=False, live=False)
        buttons = wagtail_hooks.frontend_preview_button(page=page, user=None, next_url=None, view_name="edit")
...
class TestPreviewToFrontendRedirectView:
    def test_build_route_slug_uses_nested_page_path_when_available(self):
        page = SimpleNamespace(slug="covid-19", get_url_parts=lambda request=None: (1, "https://frontend.test", "/respiratory-viruses/covid-19/"))
    ...
    def test_success_redirects(self, spy_get_object_or_404: mock.MagicMock, settings):
        ...
        settings.PAGE_PREVIEWS_FRONTEND_URL_TEMPLATE = "https://frontend.test/preview?slug={slug}&t={token}"
        request = RequestFactory().get("/cms-admin/preview-to-frontend/1/")
    ...
    @mock.patch(f"{MODULE_PATH}.dashboard.views.get_object_or_404")
    def test_success_redirects_with_nested_route_slug(self, spy_get_object_or_404: mock.MagicMock, settings):
        page = FakePage(pk=1, slug="covid-19", can_edit=True)
        page.get_url_parts = lambda request=None: (1, "https://frontend.test", "/respiratory-viruses/covid-19/")
        ...
        response = view.get(request=request, pk=1)
        location = response.url if hasattr(response, "url") else response.get("Location")
        assert location.startswith("https://frontend.test/preview?slug=respiratory-viruses%2Fcovid-19&t=")
    @mock.patch(f"{MODULE_PATH}.dashboard.views.get_object_or_404")
    def test_success_redirects_normalises_legacy_query_params(self, spy_get_object_or_404: mock.MagicMock, settings):
        spy_get_object_or_404.return_value = FakePage(pk=1, slug="cover", can_edit=True)
...
class TestAddFrontendPreviewAction:
    def test_missing_page_or_pk_returns_none(self):
        ...
        wagtail_hooks.add_frontend_preview_action(menu_items=menu_items, request=request, context={})
        menu_items_for_unsaved_page = []
        wagtail_hooks.add_frontend_preview_action(menu_items=menu_items_for_unsaved_page, request=request, context={"page": FakePage(pk=None)})
        assert menu_items == []
