...
class TestMetricsDocumentationParentPage:
    ...
    def test_has_the_correct_content_panels(
        self,
        expected_content_panel_names: str,
    ):
        """
        Given a blank `MetricsDocumentationParentPage` model.
        ...
        assert expected_content_panel_names in content_panel_names
    ...
    def test_raise_error_if_slug_not_metrics_documentation(self):
        ...
        # When / Then
        with pytest.raises(MetricsDocumentationSlugNotValidError) as exc:
            fake_metrics_documentation_parent_page._raise_error_if_slug_not_metrics_documentation()

        assert (
            "The `MetricsDocumentationParentPage` must have a slug of `metrics-documentation"
            in str(exc.value)
        )
    ...
    def test_raise_error_for_multiple_live_pages_raises_error(
        ...
        # When / Then
        with pytest.raises(MetricsDocumentationMultipleLivePagesError) as exc:
            invalid_duplicate_page._raise_error_for_multiple_live_pages()

        assert "There should only be 1 `MetricsDocumentationParentPage" in str(
            exc.value
        )
