...
class TestPopularTopicsMetricNumberBlockTypes:
    def test_metric_cards_are_required_and_allow_exactly_three_entries(self) -> None:
        """
        Given an instance of `PopularTopicsMetricNumberBlockTypes`
        When inspecting the metric_cards field configuration
        Then metric_cards are required and constrained to exactly three entries
        """
        # Given
        metric_number_block = PopularTopicsMetricNumberBlockTypes()

        # When
        metric_cards_block = metric_number_block.child_blocks["metric_cards"]

        # Then
        assert metric_cards_block.meta.required is True
        assert (
            metric_cards_block.meta.min_num
            == POPULAR_TOPICS_HEADLINE_NUMBER_BLOCK_COUNT
        )
        assert (
            metric_cards_block.meta.max_num
            == POPULAR_TOPICS_HEADLINE_NUMBER_BLOCK_COUNT
        )
