...
class TestCMSBlockParserGetAllChartCardsFromSection:
    def test_returns_correct_chart_cards(self):
        """
        Given a section containing chart blocks within chart cards
        When `get_all_chart_cards_from_section()` is called
            from the `CMSBlockParser` class
        Then the correct chart cards are returned
        """
        # Given
        section = EXAMPLE_SECTION

        # When
        chart_cards = CMSBlockParser.get_all_chart_cards_from_section(section=section)

        # Then
        expected_chart_cards = [
            # The 1st content card is a headline numbers row card, so nothing to extract there
            section["value"]["content"][1]["value"]["columns"][0],
            section["value"]["content"][1]["value"]["columns"][1],
            # The 2nd content card is a chart row card which contains 1 chart in the 2 rows.
            # So we expect to extract 2 chart cards
        ]
