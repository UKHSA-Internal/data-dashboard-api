...
class TestBarCharts:
    ...
    def test_confidence_intervals_all_data(self, fake_plot_data: PlotGenerationData):
        ...
        assert (
            tuple(fake_plot_data.additional_values.get("upper_confidence"))
            == main_bar_plot.error_y.array
        )
        assert (
            tuple(fake_plot_data.additional_values.get("lower_confidence"))
            == main_bar_plot.error_y.arrayminus
        )
