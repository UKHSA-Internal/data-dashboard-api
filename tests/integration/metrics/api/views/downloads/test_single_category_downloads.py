...
class TestDownloadsView:
    ...
    def _create_example_core_time_series(
        self, is_public: bool = True
    ) -> CoreTimeSeries:
        metric_prefix = "OFF-SENS_" if not is_public else ""
        return CoreTimeSeriesFactory.create_record(
            ...
            metric_name=metric_prefix + self.core_timeseries_data["metric"],
    ...
    def test_csv_download_non_pub_timeseries_returns_correct_response(
        ...
        # core_time_series = self._create_example_non_pub_time_series()
        # valid_payload = self._build_valid_non_pub_payload()
