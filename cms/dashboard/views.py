...
class PreviewToFrontendRedirectView(View):
    ...
    @staticmethod
    def _canonicalise_preview_url(*, raw_url: str, slug: str, token: str) -> str:
        """Return preview URL with canonical query params.

        The query string is always normalised to `slug` and `t` so stale or
        legacy template parameters (e.g. `draft=true`, `page_id`, `slug_name`)
        are not propagated to the frontend.
        """
        parts = urlsplit(raw_url)
        query = urlencode({"slug": slug, "t": token})
        return urlunsplit(
