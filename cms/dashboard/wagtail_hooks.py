...
def _build_view_live_url(page: Page) -> str | None:
    ...
    page_previews_frontend_base_url = settings.PAGE_PREVIEWS_FRONTEND_BASE_URL
    ...
    if button_label == VIEW_LIVE_LABEL:
        ...
        if live_url:
            preview_item = FrontendPreviewAction(
                url=live_url, label=button_label, order=0
            )
            menu_items.insert(0, preview_item)
    elif button_label == "Preview":  # pragma: no cover
        _insert_frontend_preview_action(
            menu_items, page, button_label
        )  # pragma: no cover
