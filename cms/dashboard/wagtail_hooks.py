...
# Move FrontendPreviewAction to module level
...
def _build_view_live_url(page: Page) -> str | None:
    # get path portion of page url else return the page slug
    try:
        _, _, page_path = page.get_url_parts(request=None)
        route_path = (page_path or "").strip("/")
    except (AttributeError, TypeError, ValueError) as e:
        route_path = str(getattr(page, "slug", ""))
    
    # live url is built from 
    # PAGE_PREVIEWS_FRONTEND_BASE_URL plus [page path | slug]
    page_previews_frontend_base_url = getattr(settings, "PAGE_PREVIEWS_FRONTEND_BASE_URL")

    if route_path is not None:
        return f"{page_previews_frontend_base_url}/{route_path}"
...
WagtailEditView.get_view_live_message_button = (
    _patched_edit_view_live_message_button
)
...
def frontend_preview_button(
    ...
    # Otherwise, use the preview redirect as before
    try:
...
def add_frontend_preview_action(
    ...
    # Hide preview and view live action if PAGE_PREVIEWS_ENABLED is False
    ...
    if button_label == "View Live":
        live_url = _build_view_live_url(page=page)
        if live_url:
            preview_item = FrontendPreviewAction(url=live_url, label=button_label, order=0)
            menu_items.insert(0, preview_item)
    elif button_label == "Preview": # pragma: no cover
        _insert_frontend_preview_action(menu_items, page, button_label) # pragma: no cover

    return  # pragma: no cover


def _insert_frontend_preview_action(menu_items, page, button_label):
    """
    Helper to insert the FrontendPreviewAction for the 'Preview' button.
    Extracted to ensure explicit branch coverage.
    """
    try:
        admin_url = reverse("cms_preview_to_frontend", args=[page.pk])
    except NoReverseMatch:
    ...
    menu_items.insert(0, preview_item)
    # No explicit return needed; Python returns None by default
