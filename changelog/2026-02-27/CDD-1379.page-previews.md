...
- Extends existing `/api/drafts/{slug}/` endpoint with token validation
...
- Route/authorization split: slug identifies the draft URL, while signed
  `page_id` in the token must match the resolved page.
...
Browser GET /admin/preview-to-frontend/{pk}/
  -> PreviewToFrontendRedirectView.get(request, pk)
      ...
      -> frontend_url = PAGE_PREVIEWS_FRONTEND_URL_TEMPLATE.format(slug=..., token=...)
