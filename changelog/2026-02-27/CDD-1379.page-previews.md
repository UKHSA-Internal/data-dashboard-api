...
Additionally, allow users to select to set "Embargo Time" by selecting a virtual date in order to preview otherwise embargoed data.
...
sequenceDiagram
    ...
    CMS-->>Browser: Render Preview button + Embargo Time picker
    Note right of Browser: Preview button and datetime picker visible
    Browser->>CMS: Select embargo timetime or 'now' in picker
    ...
    FE-->>Browser: Rendered preview page with Embargo Time data
```
...
- In the CMS, the editor selects either a specific datetime or `now` from the Embargo Time picker.
- On Preview, the redirect API includes the embargo time as `et` in the frontend redirect querystring.
- At the time of writing, the frontend reads `et` and shows a warning/banner that the page is being viewed in Embargo Time mode, including a formatted date and time.
