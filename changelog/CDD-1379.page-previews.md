...
via `PagePreviewEnabled: dict[str, bool]`.
...
* `frontend_preview_button` and `add_frontend_preview_action` both call
  `_is_preview_enabled_page(page)`.
* If the page type resolves to `True`, the custom Preview button/action is shown.
* If it resolves to `False`, no custom Preview button/action is shown.

Implementation notes:

* Page type matching is class-name based.
* The check supports subclasses (via class MRO), so test/fake classes that
  inherit from an enabled page type are treated as enabled.
* The helper avoids forcing database access in unit test scenarios where
  `page.specific_class` may not be safely available.

To enable preview for a page type:

1. Add (or update) the page class name in `PagePreviewEnabled`.
2. Set the value to `True`.
3. Run `tests/unit/cms/dashboard/test_wagtail_hooks.py` (and related preview tests)
   to confirm behavior.
