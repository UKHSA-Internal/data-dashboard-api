...
4. **Tests**
   * Enhanced existing `wagtail_hooks` tests and added new ones:
     - header‑button hook returns correct URL
     - `construct_page_action_menu` inserts preview action
     - patched `get_side_panels` removes the preview panel for composite
       pages
   * Used fake page factories to avoid database access.
