...
def group_by_geography(
    ...
    """
    Group permissions by specific geography (without geography type level).

    Simpler flat structure showing what themes/topics are available per geography.

    Args:
        permissions: List of deduplicated NormalizedPermission objects

    Returns:
        Dict mapping geography codes to their available permissions
    """
    grouped = defaultdict(lambda: {
        "geography_name": None,
        "geography_type": None,
    ...
    for perm in permissions:
        geo_code = perm.geography_id or "*"

        # Initialize geography metadata
        if not grouped[geo_code]["geography_name"]:
            grouped[geo_code]["geography_name"] = perm.geography_name
            grouped[geo_code]["geography_type"] = perm.geography_type_name
            grouped[geo_code]["geography_code"] = geo_code

        # Add theme/topic permission
        grouped[geo_code]["permissions"].append({
            "theme": {
                "id": perm.theme_id or None,
                "name": perm.theme_name or None,
            },
