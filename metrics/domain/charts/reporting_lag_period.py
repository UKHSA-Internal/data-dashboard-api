...
def _get_last_x_value_at_end_of_reporting_lag_period(
...
def _get_x_value_at_start_of_reporting_lag_period(
...
def add_reporting_lag_period(
    chart_plots_data: list[PlotData],
    figure: plotly.graph_objects.Figure,
    boundary_colour: str = CYAN,
    fill_colour: str = LIGHT_BLUE,
):
    """Adds a filled region representing the reporting lag period on the figure
    ...
    """
    start_x_of_reporting_lag: str = _get_x_value_at_start_of_reporting_lag_period(
        chart_plots_data=chart_plots_data
    )
    last_x_of_reporting_lag: str = _get_last_x_value_at_end_of_reporting_lag_period(
        figure=figure
    )

    # Draw the shaded section of the reporting lag period
    figure.add_vrect(
        x0=start_x_of_reporting_lag,
        x1=last_x_of_reporting_lag,
        fillcolor=fill_colour,
        opacity=0.6,
        line_width=0,
    )

    # Draw the boundaries of the reporting lag period
    figure.add_vline(
        x=start_x_of_reporting_lag, line_color=boundary_colour, line_width=2
    )
    figure.add_vline(
        x=last_x_of_reporting_lag, line_color=boundary_colour, line_width=2
    )
